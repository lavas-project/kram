...
import {getOne, get} from '@/assets/common/utils/basic';
import {
    rAF,
...
export default class HashScroller {
    constructor(params) {
        this.params(params);
    ...
    scrollTo(hash) {
        ...
        if (hash) {
            let item = getOne(
                ...
                item => hash === item.hash
                    || (this.shouldDecode
                        && decodeURIComponent(hash) === item.hash
                    )
            );
    ...
    getScrollHash() {
        ...
        for (let i = 0, max = heights.length; i < max; i++) {
            ...
            else if (scrollTop < heights[i].bottom) {
                return heights[i].hash;
            }
        }
