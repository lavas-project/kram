...
export default function (app) {
    const catalogModule = {
        async generate() {
            let {entryInfos, logger, store, module: {hook}, STAGES} = app;

            if (!isValidArray(entryInfos)) {
                logger.warn('[kram] no entry file information for generating catalog.');
                return [];
            }

            let entrys = await Promise.all(
                entryInfos.map(async info => {
                    let storeInfo = await store.get('doc', info.path);
            ...
            let catalog = createCatalog(entrys);
            catalog = await hook.exec(STAGES.CREATE_CATALOG, catalog, entryInfos);
            return catalog;
        }
    };
