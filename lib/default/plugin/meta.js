        ...
        on(STAGES.CREATE_CATALOG, (catalog, entryInfos) => {
            ...
            return metaInfos.reduce((catalog, info) => {
                ...
                if (isValidArray(meta.menu)) {
                    adjustCatalog(catalogItem, meta.menu);
                }
...
function renameCatalog(catalogItem, nameMap) {
    if (isValidArray(catalogItem.children)) {
...
function adjustCatalog(catalogItem, metaMenu) {
    if (!isValidArray(catalogItem.children)) {
        return;
    }
    ...
    catalogItem.children = catalogItem.children.reduce(
        (list, child) => {
            let key = removeExt(child.path, '.md');
            ...
            if (index == null) {
                ...
                if (isValidArray(metaMenuItem.children) && isValidArray(child.children)) {
                    adjustCatalog(child, metaMenuItem.children);
                }
